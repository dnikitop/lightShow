...
class midiFile:
    ...
    def killAll(self,pub1, pub2,pub3,pub4,pub5,pub6, rate, channel):
        ...
        for eve in self.xevents:
            ...
            if (eve.body):
                ...
                pub3.publish(eve.msgs)
                pub5.publish(eve.msgs)
            else:
                pub2.publish(eve.rosMessage)
                pub4.publish(eve.rosMessage)
                pub6.publish(eve.rosMessage)
            #eve.printMes()
            rate.sleep()
    def play(self, pub1, pub2, pub3,pub4,pub5,pub6, rate, channel):
        now = rospy.get_time()
        print "*******************************************************************"
        for eve in self.events:
            if(eve.channel == 0):
...
def talker(midiFiles, channel):
    print "Starting pubs"
    pub1 = rospy.Publisher('/drivers/brainstem/cmd/update_body_lights/1', MsgUpdateBodyLights, queue_size=30)
    pub2 = rospy.Publisher('/drivers/brainstem/cmd/update_tote_lights/1', MsgUpdateToteLights, queue_size=30)
    pub3 = rospy.Publisher('/drivers/brainstem/cmd/update_body_lights/2', MsgUpdateBodyLights, queue_size=30)
    pub4 = rospy.Publisher('/drivers/brainstem/cmd/update_tote_lights/2', MsgUpdateToteLights, queue_size=30)
    pub5 = rospy.Publisher('/drivers/brainstem/cmd/update_body_lights/3', MsgUpdateBodyLights, queue_size=30)
    pub6 = rospy.Publisher('/drivers/brainstem/cmd/update_tote_lights/3', MsgUpdateToteLights, queue_size=30)

    rospy.init_node('talker',anonymous=True)
    rate = rospy.Rate(200) # 10hz
    midiFiles[0].killAll(pub1,pub2,pub3,pub4,pub5,pub6,rate,channel)
    ...
    for mid in midiFiles:
        ...
        mid.play(pub1,pub2,pub3,pub4,pub5,pub6,rate,channel)
        i = i + 1
